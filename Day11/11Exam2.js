...
'스타일 변경' 버튼(id: 'styleBtn')을 클릭하면, id가 'colorBox'인 <div>의 배경색을 
'skyblue'로, 글자색을 'white'로 변경하시오.*/
...
function 토글함수(){        // 토글? 버튼처럼 on / off 하는 것.
    // 1. 특정한 마크업 DOM 가져오기.
    let 마크업 = document.querySelector('body')
    // 2. 가져온 마크업 DoM에서 토글 적용하기.  // classList : 클래스 목록 반환 
    // classList.toggle('클래스명') --> 클래스 목록에 특정 클래스명 대입과 삭제 가능.
    마크업.classList.toggle('dark-mode')
}


/*예제 5: 특정 HTML 요소 제거하기
'박스 삭제' 버튼(id: 'removeBtn')을 클릭하면, id가 'targetBox'인 
<div> 요소를 문서에서 제거하시오. */
...
function 등록함수(){
    ...
    //2. 가져온 마크업 DOM의 value속성값 가져오기.
    let 입력값 = 마크업.value;
    //3. 전역배열에 추가 (입력이 아니라 추가이므로.)
    전역배열.push(입력값)
    //4. 출력
    console.log( 전역배열 )
}